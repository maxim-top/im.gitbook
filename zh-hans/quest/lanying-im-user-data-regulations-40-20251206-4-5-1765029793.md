...
description: >-
  在现代社会中，数据隐私和安全变得愈发重要。**蓝莺IM在处理用户数据时遵循的法律法规主要包括：1、个人信息保护法；2、数据安全法；3、网络安全法。**
  其中，个人信息保护法是确保用户信息安全的基石，它规定了用户数据收集、存储和处理时的透明度与合规性。例如，蓝莺IM会在收集用户信息之前进行明确的告知，并获取用户的同意，以确保合规。此外，数据安全法要求企业建立健全的数据安全管理制度，加强对数据的保护措
---
